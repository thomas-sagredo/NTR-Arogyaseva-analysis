...
hospitales_estables <- resumen_hospital_todos %>%
  ...
  pull(nombre_hospital) #ver si cambiamos criterio para htles estables
...
#Alternativas al modelo final

# ----------------------------------------------------------
# Modelo final alternativo
# ----------------------------------------------------------

glmm_paciente_sinsexo <- glmer(
  mortalidad_binaria ~
    edad_z +
    nombre_categoria_modelo +
    log_monto_reclamado_z +
...
summary(glmm_paciente_sinsexo)

or_glmm_paciente_principal_sinsexo <- exp(cbind(
  OR = fixef(glmm_paciente_sinsexo),
  confint(glmm_paciente_sinsexo, parm = "beta_", method = "Wald")
))

or_glmm_paciente_principal_sinsexo

icc_paciente_principal_sinsexo <- calcular_icc_logistico(glmm_paciente_sinsexo)
icc_paciente_principal_sinsexo
